...
class FlowchartSerializer implements IFlowchartSerializer {
    var customParamTypeByName:Map<String, CustomParamType>;
    
    public function new(?customParamTypes:Array<CustomParamType>) {
        customParamTypeByName = new Map();
        if (customParamTypes != null) {
            for (customType in customParamTypes) {
                customParamTypeByName.set(customType.typeName, customType);
            }
        }
    }
                ...
                if (value != null) {
                     switch(param) {
                        ...
                        case Custom(_, _, custom):
                            var customType = customParamTypeByName.get(custom.typeName);
                            if (customType != null && customType.serializeValue != null) {
                                argStr = customType.serializeValue(value);
                            } else {
                                argStr = "null";
                            }
                    }
