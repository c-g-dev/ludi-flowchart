package ludi.flowchart.controller;
...
import ludi.flowchart.model.Graph.GraphConnection;
...
class FlowchartController implements IComposible {
    ...
    function init(comp:Composition<Dynamic>) {
        ...
        canvasView.onNodeCreate = function(templateName, x, y) {
            ...
            if (template != null) {
                ...
                if (template.requiredNodes != null && template.requiredNodes.length > 0) {
                    var allNodes:Array<FlowchartNode> = [node];
                    var allEdges:Array<GraphConnection<FlowchartNode, FlowchartConnection>> = [];

                    for (i in 0...template.requiredNodes.length) {
                        var req = template.requiredNodes[i];
                        var reqTemplate = findTemplate(req.templateName);
                        if (reqTemplate == null) continue;
                        var reqId = nodeId + "_req_" + i;
                        var offsetY = (i + 1) * 80;
                        var reqNode = new FlowchartNode(reqId, reqTemplate, x + 200, y + offsetY, nodeId);
                        allNodes.push(reqNode);
                        allEdges.push(new GraphConnection(node, reqNode, new FlowchartConnection(req.connectionTag)));
                    }

                    transactionManager.add(new AddNodeWithRequiredTransaction(graph, allNodes, allEdges));
                    for (n in allNodes) graph.addNode(n);
                    for (e in allEdges) graph.addEdge(e);
                } else {
        ...
        canvasView.onContextMenu = function(x, y, targetType, targetId) {
            ...
            if (targetType == "node") {
                ...
                if (targetNode != null && targetNode.ownerNodeId != null) {
                    // Required/owned nodes cannot be deleted independently
                } else {
                    items.push({
                        label: "Delete Node",
                        action: function() {
                            ...
                            if (node != null) {
                                var ownedNodes = getOwnedNodes(node.id);
                                if (ownedNodes.length > 0) {
                                    var allNodes = [node].concat(ownedNodes);
                                    var allEdges:Array<GraphConnection<FlowchartNode, FlowchartConnection>> = [];
                                    for (n in allNodes) {
                                        for (e in graph.adjacent(n)) allEdges.push(e);
                                    }
                                    transactionManager.add(new DeleteNodeWithRequiredTransaction(graph, allNodes, allEdges));
                                    for (e in allEdges) graph.removeEdge(e);
                                    for (n in allNodes) graph.removeNode(n);
                                } else {
        ...
        canvasView.onConnectionContextMenu = function(x, y, conn) {
            var isOwnershipEdge = false;
            var target:FlowchartNode = conn.target;
            if (target != null && target.ownerNodeId != null && target.ownerNodeId == conn.source.id) {
                isOwnershipEdge = true;
            }
    ...
    function getOwnedNodes(ownerId:String):Array<FlowchartNode> {
        var owned:Array<FlowchartNode> = [];
        for (n in graph.nodes) {
            if (n.ownerNodeId == ownerId) owned.push(n);
        }
        return owned;
    }
